...
void gbe_Step(void) 
{
  ...
  if (( k & MASK_JOY1_RIGHT) || ( k & MASK_JOY2_RIGHT)) hw.pad|=PAD_RIGHT;
  if (( k & MASK_JOY1_LEFT) || ( k & MASK_JOY2_LEFT)) hw.pad|=PAD_LEFT;
  if (( k & MASK_JOY1_UP) || ( k & MASK_JOY2_UP)) hw.pad|=PAD_UP;
  if (( k & MASK_JOY1_DOWN) || ( k & MASK_JOY2_DOWN)) hw.pad|=PAD_DOWN;
  if ( k & MASK_JOY2_BTN) hw.pad|=PAD_A;
  if (k & MASK_KEY_USER1)  hw.pad|=PAD_B;
